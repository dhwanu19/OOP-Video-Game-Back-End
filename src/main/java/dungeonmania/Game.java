...
public class Game {
    ...
    public Game tick(Direction movementDirection) {
        ...
        tick();
        // add logical entities ting here somewhere
        resetLogic();
        return this;
    }
    ...
    // public void registerSceptre(Scepter sceptre) {
    //     // check queue constant
    //     register(() -> sceptre.tickSceptre(this), PLAYER_MOVEMENT_CALLBACK, sceptre.getId());
    // }

    // public void unregisterSceptre(Scepter sceptre) {
    //     unsubscribe(sceptre.getId());
    // }
    ...
    public Game build(String buildable) throws InvalidActionException {
        ...
        if (!buildables.contains(buildable)) {
            ...
            throw new InvalidActionException(String.format("%s cannot be built since there are zombies", buildable));
        }
    ...
    public void setNumEnemiesDestroyed(int numEnemiesDestroyed) {
        this.numEnemiesDestroyed = numEnemiesDestroyed;
    }
