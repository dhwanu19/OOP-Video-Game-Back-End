...
public class GameMap {
    ...
    private void initLogicalObservers() {
        List<LogicalEntity> logicEntities = getEntities(LogicalEntity.class);
        logicEntities.forEach(e -> {
            List<LogicalEntity> observees = logicEntities.stream()
                    .filter(t -> Position.isAdjacent(e.getPosition(), t.getPosition())).collect(Collectors.toList());
            e.setObservers(observees);
        });
    }
    ...
    private void triggerMovingAwayEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity && e instanceof OnMovedAway) {
                OnMovedAway method = (OnMovedAway) e;
                callbacks.add(() -> method.onMovedAway(this, entity));
            }
        });
    ...
    private void triggerOverlapEvent(Entity entity) {
        ...
        getEntities(entity.getPosition()).forEach(e -> {
            if (e != entity && e instanceof OnOverlap) {
                OnOverlap method = (OnOverlap) e;
                overlapCallbacks.add(() -> method.onOverlap(this, entity));
            }
        });
    ...
    public void destroyEntity(Entity entity) {
        ...
        if (entity instanceof OnDestroy) {
            OnDestroy method = (OnDestroy) entity;
            method.onDestroy(this);
        }
    }
    ...
    public int getNumEnemiesDestroyed() {
        return game.getNumEnemiesDestroyed();
    }

    public void setNumEnemiesDestroyed(int numEnemiesDestroyed) {
        game.setNumEnemiesDestroyed(numEnemiesDestroyed);
    }
