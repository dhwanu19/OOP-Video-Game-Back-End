...
public class EntityFactory {
    ...
    public MidnightArmour buildMidnightArmour() {
        double defence = config.optDouble("midnight_armour_defence", 1);
        double attack = config.optDouble("midnight_armour_attack", 1);
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "light_bulb_off":
            return new LightBulb(pos, jsonEntity.getString("logic").toUpperCase());
        case "wire":
            return new Wire(pos);
        case "switch_door":
            return new SwitchDoor(pos, jsonEntity.getString("logic").toUpperCase());
        default:
            return null;
        }
    }
